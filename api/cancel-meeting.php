...
try {
    // Toplantının kullanıcıya ait olduğunu ve iptal edilebilir durumda olduğunu kontrol et
    $stmt = $pdo->prepare("
    ...
    // Sadece bekleyen toplantılar iptal edilebilir
    if ($meeting['status'] !== 'pending') {
        echo json_encode(['success' => false, 'message' => 'Sadece bekleyen toplantılar iptal edilebilir.']);
        exit;
    }
    ...
    if ($stmt->execute([$meetingId, $currentUser['id']])) {
        writeLog("Meeting cancelled: ID $meetingId by user " . $currentUser['id'], 'info');
    ...
    echo json_encode(['success' => false, 'message' => 'İptal işlemi sırasında hata oluştu.']);
}
?>